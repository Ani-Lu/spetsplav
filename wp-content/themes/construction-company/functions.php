...
function construction_company_enqueue_styles_and_scripts() {
    ...
    if( $primary_fonts ){
        ...
        wp_add_inline_style( 'construction-company-style', $custom_css );
    }
}
...
function construction_company_customizer_theme_info( $wp_customize ) {
    ...
    $theme_info .= '<span class="sticky_info_row"><label class="row-element">' . __( 'Theme Demo', 'construction-company' ) . ': </label><a href="' . esc_url( 'https://rarathemes.com/previews/?theme=construction-company' ) . '" target="_blank">' . __( 'Click here', 'construction-company' ) . '</a></span><br />';
...
if ( ! function_exists( 'construction_company_get_fontawesome_ajax' ) ) :
/**
 * Return an array of all icons.
 */
function construction_company_get_fontawesome_ajax() {
    // Bail if the nonce doesn't check out
    if ( ! isset( $_POST['construction_company_customize_nonce'] ) || ! wp_verify_nonce( sanitize_key( $_POST['construction_company_customize_nonce'] ), 'construction_company_customize_nonce' ) ) {
        wp_die();
    }

    // Do another nonce check
    check_ajax_referer( 'construction_company_customize_nonce', 'construction_company_customize_nonce' );
    ...
    $fonts = construction_company_get_fontawesome_list();
...
add_action( 'wp_ajax_construction_company_get_fontawesome_ajax', 'construction_company_get_fontawesome_ajax' );

function construction_company_customize_script(){
    wp_localize_script( 'construction-company-repeater', 'construction_company_customize',
        array(
            'nonce' => wp_create_nonce( 'construction_company_customize_nonce' )
        )
    );
}
add_action( 'customize_controls_enqueue_scripts', 'construction_company_customize_script' );